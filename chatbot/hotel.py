    ...
    try:
        ...
        if lat and long:
            ...
            if hotel_msg is None:
                responder.reply("Currently, We don't have any preferences for you😕 But you can always try saying find hotels near " + firebase.getDest(id)+"~"+"I will be there to help you 🙂")
            else:
...
def search_hotel_at_curr(request, responder):
    ...
    try:
        ...
        hotel_msg = hotelList(id,lat,long)
        if hotel_msg is None:
            responder.reply("We don't have any preferences for you😕. You can always try saying "+ _fetch_find_hotel_in_suggestion()["suggestion"] + firebase.getDest(id))
        else:
